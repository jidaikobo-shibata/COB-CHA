...
function getUiJa ()
{
  return {
    ...
    'config-exp': 'WCAG 2.0、2.1もしくはTrusted Testerのいずれかから試験の方式を選択し、あわせて達成レベルを選択してください。また追加の達成基準を適用する場合は、この時点で決定してください（あとから足すのはかなり手間です）。設定シートを生成して、この設定を保存するようにしてください。',
    ...
    'issue-control-exp': '「*Issue*」という名前のシートを生成し、ダイアログで入力した課題を*Issue*シートに追加します。アクティブセルが既存の問題点にある場合は、編集します',
