...
function generateSheets(urlstr, lang, testType, level) {
  ...
  if (addSheet(urls[0]) == false) return("Aborted: "+urls[0]+" is already exists");
  ...
  if (urls.length > 1) {
    for(var i = 1; i < urls.length; i++) {
      if (addSheet(urls[i], originalSheet) == false) return("Aborted: "+urls[i]+" is already exists");
  ...
  return(urls.length+" sheet(s) generated");
}
