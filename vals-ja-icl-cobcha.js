...
function getIclTestCobcha () {
  return {
    ...
    '1.3.2' : [
      ...
      ['PDF3', 'PDF文書で正しいタブ順序と読み上げ順序を確保している', true]
    ],
    ...
    '3.3.2' : [
      ...
      ['G131/G162', '可視の画像（ルーペ/虫眼鏡など）が入力すべき内容を十分に説明している（英語版3.3.2解説書の「例」参照）'],
      ...
      ['G167', '隣接するボタンが入力内容を説明している（最後の手段）'],
      ['H65', '可視ラベルを表示する領域がないときにtitle属性でラベルを提供している（非推奨）', true],
      ['G131/PDF5', '（PDFのみ）入力すべき内容を視覚的に理解できるラベルがあり、かつ必須項目が明示されている', true],
      ['PDF10', '（PDFのみ）入力すべき内容を視覚的に理解できるラベルがある', true]
    ],
    ...
    '4.1.2-C' : [
      ['G135/G10', 'HTML/JavaScript/CSSなどの一般的なウェブ技術以外（JavaやPython等）を用いてユーザインタフェースを作成しているが、ブラウザで名前や役割を理解できるようにしている'],
      ['PDF10/PDF12', 'PDFのフォームコントロールがPDFにおける標準的な技術で作成されており、名前や役割を理解できる', true]
    ],
    ...
    '1.4.11-A' : [
      ...
      ['-', 'リンク、ボタン等の枠線のコントラスト比が要件を満たさないが、枠線内の文字が1.4.3を満たす'],
    ...
    '1.4.11-B' : [
      ...
      ['-', 'グラフに続く表（table）などで、情報が他の形式で提供されている（解説書より）']
    ],
