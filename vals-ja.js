...
function getCriteriaJa ()
{
  return [
    ...
    ['AA',  '2.4.11', '隠されないフォーカス (最低限)', '', '', ''],
    ['AAA', '2.4.12', '隠されないフォーカス (高度)', '', '', ''],
    ['AAA', '2.4.13', 'フォーカスの外観', '', '', ''],
    ...
    ['AAA', '2.5.5',  'ターゲットのサイズ (高度)', '', 'target-size'],
    ['AAA', '2.5.6',  '入力メカニズム非依存', '', 'concurrent-input-mechanisms'],
    ['AA',  '2.5.7',  'ドラッグ動作 ', '', ''],
    ['AA',  '2.5.8',  'ターゲットのサイズ (最低限)', '', ''],
    ...
    ['A',   '3.3.7',  '冗長な入力項目', '', ''],
    ['AA',  '3.3.8',  'アクセシブルな認証 (最低限)', '', ''],
    ['AAA', '3.3.9',  'アクセシブルな認証 (高度)', '', ''],
...
function getTechValJa ()
{
  return {
    ...
    'ARIA22' : 'ステータスメッセージを提示するために role=status を使用する',
    ...
    'C31'    : 'コンテンツをリフローするために CSS Flexbox を使う',
    'C32'    : 'メディアクエリとグリッド CSS を使用して列をリフローする',
    'C33'    : '長い URL 及びテキスト文字列をリフローできるようにする',
    'C34'    : '固定されていないスティッキーヘッダー/フッターに、メディアクエリーを利用する',
    'C35'    : '折り返しのないテキストの間隔を見込む',
    'C36'    : 'テキスト間隔を上書きできるようにする',
    'C37'    : '画像を収めるために、CSS の max-width 及び height を使用する',
    'C38'    : 'CSS width、max-width、及び flexbox を用いたラベルと入力欄の配置',
    'C39'    : 'モーションの防止に CSS reduce-motion クエリを使用する',
    'C40'    : 'すべてのコンポーネントで十分なコントラスト比を確保するために 2 色のフォーカスインジケータを作成する',
    'C41'    : '（未訳）コンポーネント内に強力なフォーカス インジケーターを作成する',
    'C42'    : '（未訳）min-height と min-width を使用して十分なターゲット間隔を確保する',
    'C43'    : '（未訳）CSS スクロールパディングを使用してコンテンツを目立たなくする',
    ...
    'F94'    : '達成基準 1.4.4 の失敗例 － テキストのサイズ変更のためのビューポート単位を誤って使用している',
    'F95'    : '達成基準 1.4.13 の失敗例 － ホバーによって表示されるコンテンツがホバーできない',
    'F96'    : '達成基準 2.5.3 の失敗例 - アクセシブルな名前 (accessible name) が可視ラベルのテキストを含んでいない',
    'F97'    : '達成基準 1.3.4 の失敗例 － 表示の向きを縦向き (portrait) 又は横向き (landscape) に固定している',
    'F98'    : '達成基準 2.5.6 の失敗例 － タッチスクリーンデバイスにおいてインタラクションがタッチのみに限定されている',
    'F99'    : '達成基準 2.1.4 の失敗例－停止又は再割り当て不可能な文字キーショートカットが実装されている',
    'F100'   : '達成基準 1.3.4 の失敗例 － デバイスの向きを変更するように促すメッセージが表示される',
    'F101'   : '達成基準 2.5.2 の失敗例 － ダウンイベントでコントロールを動作させる',
    'F102'   : '達成基準 1.4.10 の失敗例 － コンテンツがリフローしたとき、コンテンツが消えて利用できなくなる',
    'F103'   : '達成基準 4.1.3 の失敗例 － 役割 (role) 又はプロパティを通してプログラムによる解釈が可能でないステータスメッセージを提供する',
    'F104'   : '達成基準 1.4.12 の失敗例 － テキストの間隔を調整したときにコンテンツが切り取られる又は重なる',
    'F105'   : '達成基準 2.5.1 の失敗例 － 単純なポインタによる代替なしに、軌跡ベースのジェスチャによる機能を提供している',
    'F106'   : '達成基準 2.5.4 の失敗例 － 動きによる起動を解除することができない',
    'F107'   : '達成基準 1.3.5 の失敗例 － 誤った autocomplete 属性値',
    'F108'   : '（未訳）達成基準 2.5.7 の失敗例 － ドラッグ動作を必要としない機能をユーザーが操作するためのシングル ポインタの手段が提供されていないため、ドラッグ動作が必要',
    'F109'   : '（未訳）達成基準 3.3.8 と 3.3.9 の失敗例 － パスワードまたはコードを同じ形式で再入力できない',
    'F110'   : '（未訳）達成基準 2.4.11 の失敗例 － スティッキーなフッターまたはヘッダーがフォーカスされた要素を完全に隠している',
    ...
    'G207'   : 'アイコンに対して 3:1 のコントラスト比を確保する',
    'G208'   : 'アクセシブルな名前 (accessible name) の一部として可視ラベルのテキストを含める',
    'G209'   : '隣り合う色の境界に十分なコントラストを提供する',
    'G210'   : 'ドラッグ＆ドロップ操作をキャンセルできるようにする',
    'G211'   : 'アクセシブルな名前 (accessible name) を視覚的なラベルと一致させる',
    'G212'   : 'ネイティブコントロールを使用して、アップイベントで機能がトリガーされるようにする',
    'G213'   : '動きで作動させる入力のために、従来のコントロール及びアプリケーション設定を提供する',
    'G214'   : '別の方法で制限されている、異なる向きのコンテンツへのアクセスを許可するコントロールを使用する',
    'G215'   : '軌跡ベース又はマルチポイントのジェスチャと同等の結果を得られるコントロールを提供する',
    'G216'   : 'コントロールスライダーのためにアクティブにするシングルポイントを提供する',
    'G217'   : '利用者が文字キーのショートカットを再割り当て又は無効にできるメカニズムを提供する',
    'G218'   : '（未訳）電子メールリンク認証',
    'G219'   : '（未訳）コンテンツを操作するドラッグ動作に代替手段を利用できるようにする',
    'G220'   : '（未訳）お問い合わせリンクを一貫した場所に提供する',
    'G221'   : '（未訳）プロセスの前のステップからのデータを提供する',
    'G223'   : '（未訳）作成者が提供した、視認性の高いフォーカス インジケーターを使用する',
    ...
    'H98'    : 'HTML 5.2 autocomplete 属性を使用する',
    'H99'    : '（未訳）ページ選択メカニズムを提供する',
    'H100'   : '（未訳）適切にマークアップされた電子メールとパスワードの入力を提供する',
    'H101'   : '（未訳）セマンティック HTML 要素を使用してページの領域を識別する',
    'H102'   : '（未訳）HTML ダイアログ要素を使用したモーダル ダイアログの作成',
...
function getUiJa ()
{
  return {
    ...
    'choose-target-condition' : '一括編集の対象を選択',
    ...
    'no-particular-issue-was-reported-on-this-page': 'このページに固有の問題は登録されていません',
    'no-common-issue-was-reported-on-this-page': 'このページに共通の問題は登録されていません',
    'common-issue': '共通の問題点',
    'no-common-issue-was-reported': '共通の問題は登録されていません',
    'common-issue-on-this-page': 'このページに存在する共通の問題',
    'no-issue-found': 'このページには問題が登録されていません',
    'no-target-url-found': '記録シートでないようです',
    'all-page': 'すべてのページ',
    ...
    'keyboard-operation-title': 'キーボード操作',
    'keyboard-operation-exp': '<kbd>Alt(option) + Shift + N</kbd>を押すことで、「アドオン」メニューに移動するので、そこからCOB-CHAコントロールパネルにフォーカスを移せます。また、<kbd>ESC</kbd>で、COB-CHAコントロールパネルから抜けてGoogle Spreadsheetに戻ることができます',
    'sheet-title': 'シート',
    'sheet-exp': 'シートの名前が*から始まるのは、COB-CHAの操作対象外のシートです。試験の記録のメモ用のシートなどは、「*memo」というように、*から始めるようにしてください。',
    'operation-title': '操作方法',
    'config-exp': 'WCAG 2.0、2.1、2.2もしくはTrusted Testerのいずれかから試験の方式を選択し、あわせて達成レベルを選択してください。また追加の達成基準を適用する場合は、この時点で決定してください（あとから足すのはかなり手間です）。設定シートを生成して、この設定を保存するようにしてください。',
    'set-template-exp': 'テストの種類、レベル、追加した達成基準に応じたテンプレートシートを生成できます。ウェブサイトの試験の場合、ヘッダやフッタなどの共通する箇所についての試験をテンプレートに記録してください。「4. 値の編集」で、一気に全ページの試験結果に反映することができます。テンプレートの反映は、既存の結果を上書きしてしまうので、最初に行ってください。',
    'generate-icl-template-sheet-exp': '実装チェックリストのテンプレートはカスタマイズできます。カスタマイズの際は、B列、C列をもとのテンプレートに合わせると、集計にも対応できます。',
    'target-urls-setting-exp': '「*URLs」という名前のシートを生成します。試験対象のURLをここに記入して、「URLごとの記録シートを生成する」を実行するとURLの数だけ記録シートを生成します。',
...
/**
 * COB-CHA ICL
 * key must be capitalised
 * @return Array
 */
function getIclListJa ()
{
  return {
    'Waic': {
      'label' : 'WAIC式'
