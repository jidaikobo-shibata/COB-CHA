...
function getIclSituationCobcha () {
  return {
    ...
    '2.5.3' : {
      '2.5.3-A' : 'テキストまたは文字画像による可視ラベルを持つユーザインタフェースコンポーネントがある場合（ただし記号的な文字──テキストエディタの「B」や「i」など──を除く）',
...
function getIclTestCobcha () {
  return {
    '1.1.1-A' : [
      ...
      ['H64/ARIA14/ARIA16', 'iframe/frameの内容が地図やスライド、図表などを含むが、title属性/aria-label/aria-labelledbyで内容を識別できる（COB-CHA独自）'],
    ...
    '1.1.1-B' : [
      ...
      ['G73/G74/G92', 'iframe/frameで示された地図やスライド、図表などの内容は、その付近、あるいは別のページ等に記述された長いテキストを読むことで、内容を理解できる（COB-CHA独自）'],
    ...
    '1.1.1-D1' : [
      ...
      ['H64/ARIA14/ARIA16', 'iframe/frameの内容が時間依存メディア（ライブを含む）を含むが、title属性/aria-label/aria-labelledbyで内容を識別できる（COB-CHA独自）']
    ],
    ...
    '2.1.4' : [
      ...
      ['-', '単一キーのキーボードショートカットを実装しているが、特定のユーザインタフェースコンポーネントにフォーカスが当たっているときにのみ、キーボードショートカットが有効になっている']
    ],
    ...
    // 2.4.2
    '2.4.2' : [
      ['G88/H25/PDF18/F25', 'ウェブページには説明的なタイトルがある。サイト内に同じページタイトルが存在しないとなおよい']
    ],
    ...
    // 2.5.3
    '2.5.3-A' : [
      ['G208/G211', '「アクセシブルな名前」が可視ラベルと一致するか可視ラベルで始まる（句読点/コロンの有無、大文字小文字の差異は問わない）']
    ],
    '2.5.3-B' : [
    ...
    '3.3.1' : [
      ...
      ['-', '必須項目はrequired属性が設定されており、未入力の際UAによって指摘される（COB-CHA独自）'],
...
//      ['F79', '展開/収納できるメニューはaria-expandedによってステータスを提供している'],
    ...
    '1.3.4' : [
      ...
      ['G124', 'デバイスを回転させても向きが変わらないコンテンツの場合、コンテンツの向きを変更するコントロールが提供され、機能する']
    ],
    ...
    // 1.4.11
    '1.4.11' : [
      ['G207', 'アイコンの図と地においておいて3:1のコントラスト比が確保されており、アイコンを視覚的に識別できる'],
    ...
    '2.4.6' : [
      ...
      ['G131', '提供されているラベルは、フォーム要素の目的を的確に表している']
    ],
