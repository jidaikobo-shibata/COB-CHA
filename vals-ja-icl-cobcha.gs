...
function getIclSituationCobcha () {
  return {
    '1.1.1' : {
      ...
      '1.1.1-D2' : '非テキストコンテンツに意味があるが、試験目的などのため、説明を提示しなくても良い場合',
    ...
    '1.4.1' : {
      '1.4.1' : '色の違いに依存して情報を伝える（それぞれの色に意味を割り当てた）コンテンツがある場合（visitedの色などUA提供の機能を除く）'
    },
    ...
    '2.2.2' : {
      '2.2.2' : '動き、点滅、スクロール: 動きのある、点滅している、又はスクロールしている情報が、自動的に開始し、ページにおける唯一のコンテンツでない場合'
    },
    ...
    '1.4.11' : {
      '1.4.11' : 'ユーザインタフェースコンポーネントあるいはコンテンツを理解するのに必要なグラフィックがある場合'
    },
...
function getIclTestCobcha () {
  return {
    ...
    '1.1.1-C' : [
      ['G82/H30/H37/ARIA6/ARIA10', 'a要素内のimg要素のalt属性値、aria-label、aria-labelledbyは、aの「リンク先の情報」を短いテキストで提供している（2.4.4も確認）'],
    ...
    '1.1.1-F' : [
      ['C9/H67/PDF4', '非テキストコンテンツは装飾的であり、空のalt、CSSの背景画像を用いて、支援技術が無視できるように実装されている']
    ],
    ...
    '1.3.1-A' : [
      ...
      ['H44/ARIA6/ARIA10/PDF10', 'フォームコントロールはlabel要素、aria-label、aria-labelledbyによってラベルと関連づけられている'],
      ['H65', 'フォームコントロールはtitle属性によって説明されている（非推奨）', true],
    ...
    '1.3.2' : [
      ...
      ['G57/C27/F49/F1', 'ソースコード上のコンテンツの順序が、コンテンツの視覚的提示の順序と同じである'],
    ...
    // 1.3.3
    '1.3.3' : [
      ['G96', '形や大きさや音又は位置だけでなく、知覚に依存しない説明によってもコンテンツが理解できるようになっている']
    ],

    // 1.4.1
    '1.4.1' : [
      ['G14/G205', '色の違いで伝えている情報（カレンダの色分け、色のアイコンによるコード化、必須項目など）をテキストでも利用可能にしている'],
    ...
    '2.4.1' : [
      ...
      ['H69/PDF9', '見出しタグで適切なマークが行われている'],
      ['G123', '繰り返しているコンテンツのブロックの先頭には、そのブロックの末尾へのリンクがある'],
      ['G124', 'ページの先頭に、コンテンツの各エリアへのリンクがある'],
      ['ARIA11', 'role="navigation"やrole="main"などのARIAランドマークを使用することで、ページの各領域を理解できる']
    ],

    // 2.4.2
    '2.4.2' : [
      ['G88/H25/PDF18/F25', 'ウェブページには説明的なタイトルがあり、サイト内に同じページタイトルが存在しない']
    ],
    ...
    '2.4.4' : [
      ...
      ['H77/H78/H79/H81', 'リンクの目的は、リンクテキストと、同じli、pのなかにあるか、テーブルの中にあり見出しセルとの関係が確保されているか、入れ子のリストの親のリスト項目との関係から理解できる'],
    ...
    // 3.1.1
    '3.1.1' : [
      ['H57/PDF16', '文書の初期設定言語が設定されている（HTMLの場合はhtml要素のlang属性、PDFの場合は、プロパティを確認する）']
    ],
    ...
    '3.3.2' : [
      ...
      ['H44', 'label要素によって関連づけられているラベルは可視である'],
    ...
    // 4.1.1
    '4.1.1' : [
      ['H74/H88/H93/H94/F70/F77', '（HTMLのみ）開始タグ、終了タグの使用は仕様に準拠しており、idは一意で、属性値に重複はない'],
    ...
    // 1.3.5
    '1.3.5' : [
      ['-', 'autocomplete属性において仕様に従って属性値を用いることで、個人情報を自動入力できるようになっている（H98の予定）']
    ],
    ...
    // 1.4.3
    '1.4.3' : [
      ['G18/G145', 'テキストおよび文字画像との背景の間に、すくなくとも4.5:1のコントラスト比（大きな文字の場合3:1）がある'],
    ...
    // 1.4.10
    '1.4.10' : [
      ['C32/C31/C33/C38', '400％のズームが可能なユーザエージェントでウェブページを表示し、ビューポートサイズ (CSS ピクセル単位) を幅1280、高さ1024に設定し、水平垂直の双方にスクロールバーがでないことを確認する']
    ],

    // 1.4.11
    '1.4.11' : [
      ['G195', 'マウスおよびキーボードで操作し、視認性に優れたフォーカスインジケータがあることを確認できる']
    ],

    // 1.4.12
    '1.4.12' : [
      ['C36/C35', 'ズームレベルを100％に設定し、テキストの間隔を調整できるブックマークレットやブラウザの拡張機能を使っても、文字が欠けたり、他のコンテンツに隠れることがない']
    ],

    // 1.4.13
    '1.4.13' : [
      ['-', 'ホバーで表示されるコンテンツがページの元のコンテンツを確認する又は操作することを妨げないように非表示にできる'],
      ['-', 'ホバーで表示されるコンテンツ自体もホバーを受け付けるので、マウスポインタに隠れて読めない部分がないように、マウスポインタをずらして確認できる']
    ],
    ...
    // 2.4.6
    '2.4.6' : [
      ['G130', '見出しが提供され、コンテンツの内容を表している'],
      ['G131', '入力すべき内容を理解できるラベルが提供されている']
    ],

    // 2.4.7
    '2.4.7' : [
      ['G149/G165', '標準的なリンクやフォームコントロールを使っているので、フォーカスがUAによって強調されている'],
    ...
    '3.3.3-A' : [
      ['G83/PDF5', '必須項目を空欄で送信すると、未入力の項目を特定できる説明文が表示される'],
      ['ARIA2/-', 'UAに対する正しいrequiredステートを用いている（required, aria-required）']
    ],
    ...
    // 4.1.3
    '4.1.3-A' : [
      ['ARIA22/G199', 'データが正常に送信されたときに、フィードバックを提供する']
    ],

    '4.1.3-B' : [
      ['ARIA19/G83/G84/G85/G177/G194', 'ステータスメッセージが提案、又はエラーの存在に関する警告を伝えている']
    ],
    ...
    '1.4.6' : [
      ['G17/G18', 'テキストおよび文字画像との背景の間に、すくなくとも7:1のコントラスト比（大きな文字の場合4.5:1）がある'],
    ...
    '1.4.9' : [
      ['C22/G140', 'テキストの視覚的提示はCSSで制御されている'],
      ['PDF7', 'PDFとして画像化された文字はOCRでテキストデータに変換可能である（全て画像のPDFはそもそも作成しないことが望ましい）'],
      ['-', '文字画像はロゴあるいは、書体見本や手紙の写し、署名など画像であることが必然的である']
    ],
    ...
    '2.2.6' : [
      ['-', 'セッションタイムアウトは、無操作状態が少なくとも 20 時間続く場合に発生するよう設定されている'],
      ['-', '利用者が入力したデータを 20 時間以上保持している'],
    ...
    '2.3.3' : [
      ['C39/-', 'インタラクションによって引き起こされる必要不可欠でないアニメーションを停止できる']
    ],
    ...
    '2.5.6' : [
      ['-', 'JavaScript において、高レベルの、特定の入力方法に依存しないイベントハンドラ (focus、blur、click) のみを使用する'],
